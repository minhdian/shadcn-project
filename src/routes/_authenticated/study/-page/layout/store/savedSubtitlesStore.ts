...
class SavedSubtitlesStore {
  ...
  updateCurrentSubtitle = (
    ...
    for (let i = 0; i < subtitles.length; i++) {
      ...
      const nextStartTime = nextSubtitle
        ? this.timeToSeconds(nextSubtitle.start)
        : Infinity
  ...
  handleSubtitleClick = (subtitle: (typeof subtitles)[0]) => {
    ...
    // Seek video player
    if (
      favoriteStore.currentLesson?.type === 'video' &&
      playerStore.playerRef
    ) {
  ...
  handlePlaySavedSubtitle = (savedSubtitle: any) => {
    ...
    const { playerRef, audioRef, setPlayerCurrentTime, virtuosoRef } =
      playerStore
    // 1. Tìm target course từ courses array
    console.log(
      'Available courses:',
      favoriteStore.courses.map((c) => ({ id: c.id, title: c.title }))
    )
    const targetCourse = favoriteStore.courses.find(
      (c) => c.id === savedSubtitle.courseId
    )

    if (!targetCourse) {
      console.log(
        'Available course IDs:',
        favoriteStore.courses.map((c) => c.id)
      )
      return
    }
    ...
    console.log(
      'Available chapters in course:',
      targetCourse.chapters.map((c) => ({ id: c.id, title: c.title }))
    )
    const targetChapter = targetCourse.chapters.find(
      (c) => c.id === savedSubtitle.chapterId
    )

    if (!targetChapter) {
      console.log(
        'Available chapter IDs:',
        targetCourse.chapters.map((c) => c.id)
      )
      return
    }
    ...
    console.log(
      'Available lessons in chapter:',
      targetChapter.lessons.map((l) => ({ id: l.id, title: l.title }))
    )
    const targetLesson = targetChapter.lessons.find(
      (l) => l.id === savedSubtitle.lessonId
    )

    if (!targetLesson) {
      console.log(
        'Available lesson IDs:',
        targetChapter.lessons.map((l) => l.id)
      )
      return
    }
